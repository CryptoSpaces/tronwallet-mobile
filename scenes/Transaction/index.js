...
class TransactionScene extends Component {
  ...
  submitTransaction = async () => {
    ...
    try {
      ...
      this.setState({ loadingSubmit: false, submitted: true, submitError: error.message })
    }
  }
      ...
      if (ctr === 'amount') {
        contractsElements.push(
          <Utils.Row key={ctr} style={{ justifyContent: 'space-between', marginVertical: 5 }}>
            <Utils.Text secondary size='small'>{firstLetterCapitalize(ctr)}</Utils.Text>
  ...
  renderRetryConnection = () => (
    <Utils.Content align='center' justify='center'>
      <Utils.Text size='small'>It seems that you are disconnected
  ...
  render() {
    ...
    return (
      <Utils.Container>
        ...
        <Utils.Content align='center' justify='center'>
          {submitError && <Utils.Error>Transaction Failed: {submitError}</Utils.Error>}
        </Utils.Content>
      </Utils.Container>
    )
  }
